...
@section('title', 'Edit Review - Reloved')

@section('content')
<div class="container mx-auto px-8 py-8">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-3xl font-bold text-text-primary mb-8">Edit Review</h1>
        ...
        <div class="bg-white rounded-8 border border-border p-8">
            ...
            <form method="POST" action="{{ route('reviews.update', $review->id) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="flex gap-2" id="rating-container">
                        @for($i = 1; $i <= 5; $i++)
                            <button 
                                type="button" 
                                onclick="setRating({{ $i }})"
                                class="rating-star w-12 h-12 {{ $i <= $review->rating ? 'text-yellow-400' : 'text-gray-300' }} hover:text-yellow-400 transition"
                                data-rating="{{ $i }}"
                            >
                ...
                <div>
                    <label class="block text-sm font-medium text-text-secondary mb-2">Foto Saat Ini</label>
                    <div class="grid grid-cols-5 gap-2">
                        @foreach($review->images as $image)
                            <div class="relative">
                                <img src="{{ $image }}" alt="Review Image" class="w-full h-20 object-cover rounded-8 border border-border">
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <!-- New Images -->
                <div>
                    <label class="block text-sm font-medium text-text-secondary mb-2">Tambah Foto (Opsional, Maksimal 5 total)</label>
        ...
        Array.from(input.files).forEach((file, index) => {
            ...
            reader.onload = function(e) {
                ...
                div.innerHTML = `
                    ...
                    <span class="absolute top-1 right-1 bg-black/50 text-white text-xs px-1 py-0.5 rounded">Baru</span>
                `;
