                    ...
                    @auth
                        ...
                        @else
                            ...
                            <form method="GET" action="{{ route('chat.create') }}" class="w-full">
                                ...
                                <button 
                                    id="favorite-btn"
                                class="w-full {{ $isFavorite ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-gray-100 text-text-secondary hover:bg-gray-200' }} px-6 py-3 rounded-10 font-semibold text-sm transition" 
                    ...
                    <form method="POST" action="{{ route('transactions.store') }}" id="mark-as-sold-form">
                        ...
                        <div id="shipping-fields" class="mb-6 hidden">
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label for="origin_province" class="block text-sm font-medium text-text-primary mb-2">Provinsi Asal <span class="text-red-500">*</span></label>
                                    <select 
                                        id="origin_province" 
                                ...
                                <div>
                                    <label for="origin_city_select" class="block text-sm font-medium text-text-primary mb-2">Kota Asal <span class="text-red-500">*</span></label>
                                    <select 
                                        id="origin_city_select" 
                                        class="w-full h-10 px-4 py-2 border border-border rounded-10 text-sm text-text-primary focus:ring-2 focus:ring-primary focus:border-primary transition"
                                        required
                                        disabled
                                    >
                                    ...
                                    <input type="hidden" id="origin_city_id" name="origin_city_id">
                                    <input type="hidden" id="origin_city_name" name="origin_city_name">
                                </div>
                                
                                <!-- Destination City (Kota Tujuan) -->
                                <div>
                                    <label for="destination_province" class="block text-sm font-medium text-text-primary mb-2">Provinsi Tujuan <span class="text-red-500">*</span></label>
                                    <select 
                                ...
                                <div>
                                    <label for="destination_city_select" class="block text-sm font-medium text-text-primary mb-2">Kota Tujuan <span class="text-red-500">*</span></label>
                                    <select 
                                        id="destination_city_select" 
                                        class="w-full h-10 px-4 py-2 border border-border rounded-10 text-sm text-text-primary focus:ring-2 focus:ring-primary focus:border-primary transition"
                                        required
                                        disabled
                                    >
                                    ...
                                    <input type="hidden" id="destination_city_name" name="destination_city_name">
                                </div>
                                
                                <div>
                                    <label for="weight" class="block text-sm font-medium text-text-primary mb-2">Berat (gram) <span class="text-red-500">*</span></label>
                                    <input 
                                        type="number" 
                                        id="weight" 
                                        name="weight" 
                                        ...
                                        class="w-full h-10 px-4 py-2 border border-border rounded-10 text-sm text-text-primary placeholder:text-placeholder focus:ring-2 focus:ring-primary focus:border-primary transition"
                                    >
                                    <p class="text-xs text-gray-500 mt-1">Berat produk dalam gram (minimal 1 gram)</p>
                                </div>
                                
                                <button 
                                    type="button" 
                                    id="check-shipping-btn"
                                    onclick="checkShippingCost()" 
                                    class="w-full bg-blue-500 text-white px-4 py-2.5 rounded-10 font-semibold text-sm hover:bg-blue-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
                                    disabled
                                >
                                    Cek Ongkir
                                </button>
                                ...
                                <div id="shipping-cost-results" class="hidden">
                                    <p class="font-semibold text-gray-900 mb-3 text-sm">Pilih Layanan Pengiriman:</p>
                                    <div id="shipping-options" class="space-y-2 max-h-64 overflow-y-auto border border-gray-200 rounded-10 p-2">
                                        <!-- Results will be inserted here -->
                                    </div>
                                    <p class="text-xs text-gray-500 mt-2">Pilih salah satu layanan di atas untuk melanjutkan</p>
                                </div>
...
async function toggleFavorite() {
    ...
    try {
        ...
        if (data.success) {
            // Update button state
            if (data.is_favorite) {
                favoriteBtn.className = 'w-full bg-red-500 text-white hover:bg-red-600 px-6 py-3 rounded-10 font-semibold text-sm transition';
                favoriteText.textContent = 'Hapus dari Favorit';
            } else {
                favoriteBtn.className = 'w-full bg-gray-100 text-text-secondary hover:bg-gray-200 px-6 py-3 rounded-10 font-semibold text-sm transition';
                favoriteText.textContent = 'Simpan ke Favorit';
            }
        } else {
            alert(data.error || 'Gagal mengupdate favorit');
        }
    } catch (error) {
...
async function loadProvinces() {
    try {
        ...
        if (data.success && data.data) {
            ...
            const destinationProvince = document.getElementById('destination_province');
            
            // Clear existing options except first
            originProvince.innerHTML = '<option value="">Pilih Provinsi Asal</option>';
            destinationProvince.innerHTML = '<option value="">Pilih Provinsi Tujuan</option>';
            
            data.data.forEach(province => {
                const option1 = document.createElement('option');
                option1.value = province.id;
                option1.textContent = province.name;
                originProvince.appendChild(option1);
                
                const option2 = document.createElement('option');
                option2.value = province.id;
                option2.textContent = province.name;
                destinationProvince.appendChild(option2);
            });
        }
    } catch (error) {
...
document.getElementById('origin_province')?.addEventListener('change', async function() {
    ...
    try {
        ...
        if (data.success && data.data) {
            citySelect.innerHTML = '<option value="">Pilih Kota Asal</option>';
    ...
    updateCheckShippingButton();
});

document.getElementById('destination_province')?.addEventListener('change', async function() {
    const provinceId = this.value;
...
document.getElementById('origin_city_select')?.addEventListener('change', function() {
    const selectedOption = this.options[this.selectedIndex];
    document.getElementById('origin_city_id').value = this.value;
    document.getElementById('origin_city_name').value = selectedOption.textContent;
    updateCheckShippingButton();
});

document.getElementById('destination_city_select')?.addEventListener('change', function() {
    const selectedOption = this.options[this.selectedIndex];
    document.getElementById('destination_city_id').value = this.value;
    document.getElementById('destination_city_name').value = selectedOption.textContent;
    updateCheckShippingButton();
});

// Update check shipping button state
function updateCheckShippingButton() {
    const originCity = document.getElementById('origin_city_select').value;
    const destinationCity = document.getElementById('destination_city_select').value;
    const weight = document.getElementById('weight').value;
    const checkBtn = document.getElementById('check-shipping-btn');
    
    if (originCity && destinationCity && weight && parseFloat(weight) > 0) {
...
document.getElementById('weight')?.addEventListener('input', updateCheckShippingButton);
...
function toggleShippingFields() {
    ...
    if (dealMethod === 'meetup') {
        ...
        document.getElementById('destination_city_name').value = '';
    } else {
...
function validateForm() {
    ...
    if (dealMethod === 'shipping') {
        ...
        const originCityId = document.getElementById('origin_city_id').value;
        const destinationCityId = document.getElementById('destination_city_id').value;
        isValid = isValid && shippingCost && shippingCourier && shippingService && originCityId && destinationCityId;
    }
...
async function checkShippingCost() {
    const originCityId = document.getElementById('origin_city_select').value;
    const originCityName = document.getElementById('origin_city_name').value;
    const destinationCityId = document.getElementById('destination_city_select').value;
    const destinationCityName = document.getElementById('destination_city_name').value;
    const weight = document.getElementById('weight').value;

    if (!originCityId || !destinationCityId || !weight || parseFloat(weight) <= 0) {
        alert('Harap lengkapi semua field untuk cek ongkir.');
        return;
    }

    const button = document.getElementById('check-shipping-btn');
    const originalText = button.textContent;
    button.textContent = 'Memproses...';
    button.disabled = true;

    try {
        // Get origin subdistrict ID for RajaOngkir
        ...
        if (!originData.success || !originData.subdistrict_id) {
            let message = '';
            if (originData.error_type === 'rate_limit') {
                message = `Quota RajaOngkir sudah habis untuk hari ini.\n\n${originData.message || 'Silakan coba lagi besok atau hubungi administrator.'}`;
            } else {
                message = `Kota asal "${originCityName}" tidak ditemukan.`;
        ...
        // Get destination subdistrict ID for RajaOngkir
        ...
        if (!destinationData.success || !destinationData.subdistrict_id) {
            let message = '';
            if (destinationData.error_type === 'rate_limit') {
                message = `Quota RajaOngkir sudah habis untuk hari ini.\n\n${destinationData.message || 'Silakan coba lagi besok atau hubungi administrator.'}`;
            } else {
                message = `Kota tujuan "${destinationCityName}" tidak ditemukan.`;
